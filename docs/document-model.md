...
Кроме файла (`Сохранить в файл` / `Открыть…`) документ автоматически сохраняется в IndexedDB браузера (`src/storage/`, библиотека `idb-keyval`): база `cutline`, store `documents`, одна запись по ключу `"current"`:

```ts
interface StoredSession {
  doc: CutlineDocument;
  view: { mode: "design" | "data"; recordIndex: number };
  savedAt: number;            // Date.now()
}
```

Документ кладётся объектом (структурное клонирование), не JSON-строкой. При загрузке `doc` проходит тот же `validateDocument`, что и файл; битая запись открывает пустой лист с объяснением, а не ломает редактор. Не сохраняются: история undo, зум, выделение.
